...
try {
    ...
    $result = $recipeCategories->deleteRecipeCategory($connection, $recipe_id, $category_id);

    echo json_encode(['message' => 'Recipe category deleted successfully', 'result' => $result]);

} catch (Exception $e) {
    $audit = new Audit($connection);
    $audit->record(null, 'DELETE', $e->getMessage(), $_SERVER['REMOTE_ADDR']);
